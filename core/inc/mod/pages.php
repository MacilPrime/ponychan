	...
	foreach ($boards as $board) {
		...
		while ($post = $query->fetch(PDO::FETCH_ASSOC)) {
			if (!$post['thread']) {
				...
				$po = new Thread(
					$post['id'], $post['subject'], $post['email'], $post['email_is_skype'], $post['email_is_skype'], $post['name'], $post['trip'], $post['capcode'], $post['body'], $post['time'],
				...
				$po = new Post(
					$post['id'], $post['thread'], $post['subject'], $post['email'], $post['email_is_skype'], $post['email_is_skype'], $post['name'], $post['trip'], $post['capcode'],
		...
		foreach ($boards as $board) {
			...
			while ($post = $query->fetch(PDO::FETCH_ASSOC)) {
				if (!$post['thread']) {
					...
					$po = new Post(
						$post['id'], $post['thread'], $post['subject'], $post['email'], $post['email_is_skype'], $post['name'], $post['trip'], $post['capcode'],
						$post['body'], $post['time'], $post['thumb'], $post['thumb_uri'], $post['thumbwidth'], $post['thumbheight'],
						$post['file'], $post['file_uri'], $post['filewidth'],
						$post['fileheight'], $post['filesize'], $post['filename'], $post['ip'],  $post['embed'], '?/', $mod, $post['mature']
					);
